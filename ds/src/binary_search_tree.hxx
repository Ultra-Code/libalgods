...
namespace ds
{
  class bsearch_tree
  {
    ...
    [[nodiscard]] auto
    find_subtree(tree_node *current, int node_value) const noexcept
        -> tree_node *
    {
    ...
    auto
    insert_subtree(tree_node *current, int node_value) noexcept -> bool
    {
  ...
  public:
    ...
    [[nodiscard]] auto
    find(int node_value) const noexcept -> tree_node *
    {
      return find_subtree(root_node(), node_value);
    }

    auto
    insert(int node_value) noexcept
    {
      return insert_subtree(root_node(), node_value);
    }

    static auto
    successor(tree_node *root) noexcept -> tree_node *
    {
    ...
    static auto
    level_order_traversal(tree_node *node) noexcept
    {
      if (node == nullptr)
        {
          return;
